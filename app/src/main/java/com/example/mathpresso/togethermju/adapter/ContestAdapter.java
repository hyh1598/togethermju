...
public class ContestAdapter extends CustomAdapter<Contest, ContestAdapter.ViewHolder> {
    private static final int UNSELECTED = -1;
    OnContestSelectedListener mListener;
    Activity mActivity;


    public interface OnContestSelectedListener {
        public void onSelect(Contest tutor);
    }

    public ContestAdapter(RecyclerView recyclerView, ArrayList<Contest> data, Activity context, OnContestSelectedListener listener) {
        super(context, data);
        mListener = listener;
        mActivity = context;
    }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        final Contest item = mItems.get(position);
