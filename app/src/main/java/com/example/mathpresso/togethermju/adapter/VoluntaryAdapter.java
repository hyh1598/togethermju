...
public class VoluntaryAdapter extends CustomAdapter<Voluntary, VoluntaryAdapter.ViewHolder> {
    private static final int UNSELECTED = -1;
    OnVoluntarySelectedListener mListener;
    Activity mActivity;


    public interface OnVoluntarySelectedListener {
        public void onSelect(Voluntary tutor);
    }

    public VoluntaryAdapter(RecyclerView recyclerView, ArrayList<Voluntary> data, Activity context, OnVoluntarySelectedListener listener) {
        super(context, data);
        mListener = listener;
        mActivity = context;
    }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        final Voluntary item = mItems.get(position);
